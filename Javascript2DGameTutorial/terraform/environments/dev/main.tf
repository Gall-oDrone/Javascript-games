...
resource "aws_acm_certificate" "game_certificate" {
...
resource "aws_acm_certificate_validation" "game_certificate_validation" {
  count = var.domain_name != "" ? 1 : 0
  
  certificate_arn         = aws_acm_certificate.game_certificate[0].arn
...
resource "aws_route53_record" "certificate_validation" {
  for_each = var.domain_name != "" ? {
    for dvo in aws_acm_certificate.game_certificate[0].domain_validation_options : dvo.domain_name => {
  ...
  zone_id         = data.aws_route53_zone.game_zone[0].zone_id
}

# Route53 hosted zone data source
data "aws_route53_zone" "game_zone" {
  count = var.domain_name != "" ? 1 : 0
...
module "application" {
  ...
  domain_name         = var.domain_name
  acm_certificate_arn = var.domain_name != "" ? aws_acm_certificate.game_certificate[0].arn : ""
  ...
  depends_on = [
    ...
    module.addons,
    time_sleep.wait_for_eks,
    aws_acm_certificate_validation.game_certificate_validation
  ]
}