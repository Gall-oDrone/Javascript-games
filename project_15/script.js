window.addEventListener('load', function(){
    ...
    class Player {
        ...
        update() {
            this.dx = this.speedX = (this.game.mouse.x - this.collisionX) / 20;
            this.dy = this.speedY = (this.game.mouse.y - this.collisionY) / 20;
            //sprite animation
    ...
    class Game {
        ...
        render(context, deltaTime){
            ...
            context.fillText('Score: ' + this.score, 25, 100);
            if (this.debug) {
                context.fillText('Lost: ' + this.lostHatchlings, 25, 150);
            }
            context.restore();
        ...
        init(){
            ...
            while (this.obstacles.length < this.numberOfObstacles && attempts < 500) {
                ...
                this.obstacles.forEach(obstacle => {
                    ...
                    obstacle.collisionY;
                    const distance = Math.hypot(dy,dx);
                    const distanceBufer = 150;
                    const sumOfRadii = testObstacle.collisionRadius + obstacle.collisionRadius + distanceBufer;
                    if (distance < sumOfRadii) {
                        overlap = true;
                    }
                });
                const margin = testObstacle.collisionRadius * 2;
                // TOREVIEW: obstacles images dont appear. Check conditions
